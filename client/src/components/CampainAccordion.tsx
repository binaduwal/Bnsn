...
//  },[campaigns])
  ...
  return (
    <div className="space-y-4">
      {campaigns?.[0]?.subCategories.map((campaign, index) => (
        <div
          key={campaign._id}
                ...
                >
                  <ChevronDown
                    className={`w-4 h-4 text-blue-500 transition-transform duration-300 ${
                      selectedCampaign === (campaign.alias || campaign.title) ? "rotate-180" : ""
                    }`}
                  />
                </button>
              </div>
            ...
            <div className="px-4 pb-4 space-y-2 bg-gradient-to-b from-blue-50/20 to-purple-50/20">
              {campaign.thirdCategories.map((third, i) => (
                <button
                  ...
                  className={`group w-full relative overflow-hidden transition-all duration-300 rounded-lg ${
