...
const ProjectsPage: React.FC = () => {
  ...
  const handleSelect = (blueprint: string) => {
  ...
  const ProjectRow: React.FC<{ project: Project }> = ({ project }) => (
    <div className="flex items-center justify-between py-4 px-6 group border-b border-gray-200 hover:bg-gray-50 group">
      <div className="flex items-center space-x-4">
        ...
        <span className="text-gray-900  font-semibold group-hover:text-blue-700 ">{project.name}</span>
      </div>
      ...
      <div className=" max-w-7xl mx-auto  ">
        ...
        <div className="bg-white  shadow-sm border border-gray-200 mb-6">
          ...
          <div className="px-6 py-4  flex items-center gap-2 justify-between">
            ...
            {/* dropdown  */}
            <div className="w-52 h-[48px]  relative">
              {/* Dropdown Header */}
              <div
                className="border border-gray-300 h-full rounded px-4 py-2  bg-white cursor-pointer flex items-center justify-between hover:border-gray-400 transition-colors"
                onClick={() => setIsOpen(!isOpen)}
              >
                <span className="text-gray-600">
                  {selectedBlueprint || "Filter by Blueprint"}
                </span>
                {isOpen ? (
